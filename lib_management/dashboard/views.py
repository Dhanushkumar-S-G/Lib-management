        ...
        if student.is_admin:
            if request.method == "POST" and 'issue' in request.POST:
                ...
                iss = Issue.objects.raw(
                    ...
                    0]
                if iss.cou == 2:
                    messages.error(request, roll_no + " has already taken two books")
                else:
                    try:
                        ...
                        if check.status == 'Not Available':
                            ...
                            issue = Issue(bookid=book, roll_no=roll_no, copy_no=copy_no)
                            issue.save()
                            messages.success(request,
                                             "Copy number " + copy_no + " of bookid " + book_id + " has been issued successfully to " + roll_no)
                            copy = Copy.objects.filter(bookid=book_id, copy_no=copy_no)[0]
                ...
                copy_no = request.POST.get('copy_no')
                try:
                    iss = Issue.objects.get(roll_no=roll_no, bookid=book_id, copy_no=copy_no, date_returned=None)
            ...
            to_be_returned = Issue.objects.filter(date_issued=datetime.date.today() - datetime.timedelta(days=0))
            over_due = Issue.objects.raw("select *, julianday(date()) - julianday(date_issued) as difference from dashboard_issue where date_returned is NULL and difference > 0;")
            # books that are in overdue
            return render(request, 'dashboard/ad_dashboard.html',
                          {'student': student, 'to_be_returned': to_be_returned, 'over_due': over_due})

        else:
            iss = Issue.objects.filter(roll_no=student.roll_no, date_returned=None)
            return render(request, 'dashboard/dashboard.html', {'student': student, 'iss': iss})
    else:
        return redirect('/')
...
def renew(request):
    if request.session.get('roll_no'):
        ...
        if request.method == "POST":
            ...
            try:
                iss = Issue.objects.get(bookid=book_id, roll_no=roll_no, copy_no=copy_no, date_returned =None)
                if iss.renewed == False:
                    iss.renewed = True
                    iss.date_renewed = datetime.date.today()
                    iss.save()
                    messages.success(request, "You have renewed the book successfully")
                else:
                    messages.error(request, "You have renewed the book already")
            except:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        d.save()
                        messages.success(request, "Your Damage report has been updated successfully")
                    except Book.DoesNotExist:
                        messages.error(request, "Please enter a valid book number")
